...
export default function AddPostModal({ isOpen, onClose, onPostAdded, user }: AddPostModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!content.trim() || !category.trim() || user?.id !== '7ee81f6a-f817-4ef7-8947-75bb21901615') return;
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          vendor_id: null, // Admin post, no specific vendor
          content,
          category,
